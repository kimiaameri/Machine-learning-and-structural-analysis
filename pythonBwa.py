...
with open(outputFile,'w') as outFile:
    ...
    with open(inputFile) as csv_file:
        ...
        for row in csv_reader:
            if count !=0:
                outFile.write(f'sed -E "s/^((@|\+)*RR[^.]+\.[^.]+)\.(1|2|3|4|5)/\\1/" row[0]-R1.paired.fq > row[0]-R1.paired.fq\n')
                outFile.write(f'sed -E "s/^((@|\+)*RR[^.]+\.[^.]+)\.(1|2|3|4|5)/\\1/" row[0]-R2.paired.fq > row[0]-R2.paired.fq\n')
