...
with open(outputFile,'w') as outFile:
    ...
    with open(inputFile) as csv_file:
        ...
        for row in csv_reader:
            if count !=0:
               outFile.write(f'{minicondaBin}trimmomatic PE -threads 4 -phred33 -trimlog $WORK/SANVA-outputs/trimmomatic/trimlog/{row[0]}.trimlog $WORK/SNP/data/{row[0]}_1.fastq $WORK/SNP/data/{row[0]}_2.fastq $WORK/SNP-outputs/trimmomatic/{row[0]}-R1.paired.fq $WORK/SNP-outputs/trimmomatic/{row[0]}-R1.unpaired.fq $WORK/SNP-outputs/trimmomatic/{row[0]}-R2.paired.fq $WORK/SNP-outputs/trimmomatic/{row[0]}-R2.unpaired.fq SLIDINGWINDOW:4:15 MAXINFO:50:0.5 LEADING:3 TRAILING:3 MINLEN:100\n')
